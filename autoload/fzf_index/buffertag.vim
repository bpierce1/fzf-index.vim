" =============================================================================
" File:          autoload/fzf_index/buffertag.vim
...
" Init {{{1
if exists('g:loaded_fzf_index_buftag') && g:loaded_fzf_index_buftag
	fini
en
let g:loaded_fzf_index_buftag = 1

cal add(g:fzf_index_ext_vars, {
	\ 'init': 'fzf_index#buffertag#init(s:crfile)',
	\ 'accept': 'fzf_index#buffertag#accept',
	\ 'lname': 'buffer tags',
	\ 'sname': 'bft',
	\ 'exit': 'fzf_index#buffertag#exit()',
	\ 'type': 'tabs',
	\ 'opts': 'fzf_index#buffertag#opts()',
	\ })

let s:id = g:fzf_index_builtins + len(g:fzf_index_ext_vars)

let [s:pref, s:opts] = ['g:fzf_index_buftag_', {
	\ 'systemenc': ['s:enc', &enc],
	\ 'ctags_bin': ['s:bin', ''],
	\ 'types': ['s:usr_types', {}],
	\ }]
...
fu! s:process(fname, ftype)
	...
	if has_key(g:fzf_index_buftags, a:fname)
		\ && g:fzf_index_buftags[a:fname]['time'] >= ftime
		let lines = g:fzf_index_buftags[a:fname]['lines']
	el
		...
		cal extend(g:fzf_index_buftags, cache)
	en
	retu lines
endf
...
fu! s:syntax()
	if !fzf_index#nosy()
		cal fzf_index#hicheck('CtrlPTagKind', 'Title')
...
endf
" Public {{{1
fu! fzf_index#buffertag#init(fname)
	let bufs = exists('s:btmode') && s:btmode
		\ ? filter(fzf_index#buffers(), 'filereadable(v:val)')
...
fu! fzf_index#buffertag#accept(mode, str)
	let vals = matchlist(a:str,
		\ '\v^[^\t]+\t+[^\t|]+\|(\d+)\:[^\t|]+\|(\d+)\|\s(.+)$')
	let bufnr = str2nr(get(vals, 1))
	if bufnr
		cal fzf_index#acceptfile(a:mode, bufnr)
...
fu! fzf_index#buffertag#cmd(mode, ...)
	let s:btmode = a:mode
	if a:0 && !empty(a:1)
		let s:btmode = 0
