" =============================================================================
" File:          autoload/fzf_index/mrufiles.vim
...
fu! fzf_index#mrufiles#opts()
	let [pref, opts] = ['g:fzf_index_mruf_', {
		\ 'max': ['s:max', 250],
		\ 'include': ['s:in', ''],
		\ 'exclude': ['s:ex', ''],
		...
		\ 'map_string': ['g:fzf_index_mruf_map_string', s:mruf_map_string],
		\ }]
	for [ke, va] in items(opts)
		let [{va[0]}, {pref.ke}] = [pref.ke, exists(pref.ke) ? {pref.ke} : va[1]]
	endfo
endf
cal fzf_index#mrufiles#opts()
" Utilities {{{1
fu! s:excl(fn)
...
fu! s:mergelists()
	let diskmrufs = fzf_index#utils#readfile(fzf_index#mrufiles#cachefile())
...
fu! s:reformat(mrufs, ...)
	let cwd = getcwd()
	let cwd .= cwd !~ '[\/]$' ? fzf_index#utils#lash() : ''
	if {s:re}
	...
	en
	retu map(a:mrufs, g:fzf_index_mruf_map_string)
endf
...
fu! s:addtomrufs(fname)
	let fn = fnamemodify(a:fname, get(g:, 'fzf_index_tilde_homedir', 0) ? ':p:~' : ':p')
...
fu! s:savetofile(mrufs)
	cal fzf_index#utils#writecache(a:mrufs, s:cadir, s:cafile)
endf
" Public {{{1
fu! fzf_index#mrufiles#refresh(...)
	let mrufs = s:mergelists()
	cal filter(mrufs, '!empty(fzf_index#utils#glob(v:val, 1)) && !s:excl(v:val)')
	if exists('+ssl')
...
fu! fzf_index#mrufiles#remove(files)
	let mrufs = []
	if a:files != []
		let mrufs = s:mergelists()
...
fu! fzf_index#mrufiles#add(fn)
	if !empty(a:fn)
		cal s:addtomrufs(a:fn)
	en
endf

fu! fzf_index#mrufiles#list(...)
	retu a:0 ? a:1 == 'raw' ? s:reformat(s:mergelists(), a:1) : 0
		\ : s:reformat(s:mergelists())
endf

fu! fzf_index#mrufiles#bufs()
	retu s:mrbs
endf

fu! fzf_index#mrufiles#tgrel()
	let {s:re} = !{s:re}
endf

fu! fzf_index#mrufiles#cachefile()
	if !exists('s:cadir') || !exists('s:cafile')
		let s:cadir = fzf_index#utils#cachedir().fzf_index#utils#lash().'mru'
		let s:cafile = s:cadir.fzf_index#utils#lash().'cache.txt'
	en
	retu s:cafile
endf

fu! fzf_index#mrufiles#init()
