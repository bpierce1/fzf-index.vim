" =============================================================================
" File:          autoload/fzf_index.vim
" Description:   Fuzzy file, buffer, mru, tag, etc finder.
" Author:        fzf_index Dev Team
...
let [s:pref, s:bpref, s:opts, s:new_opts, s:lc_opts] =
	...
	\ 'highlight_match':       ['s:mathi', [1, 'fzf_indexMatch']],
...
if !exists('g:fzf_index_types')
	let g:fzf_index_types = s:types
el
	call filter(g:fzf_index_types, "index(['fil', 'buf', 'mru'], v:val)!=-1")
en
let g:fzf_index_builtins = len(g:fzf_index_types)-1
...
let s:coretypes = map(copy(g:fzf_index_types), '[s:coretype_names[v:val], v:val]')
...
fu! s:opts(...)
	...
	if a:0 && a:1 != {}
		...
		for [ke, va] in items(a:1)
			let opke = substitute(ke, '\(\w:\)\?fzf_index_', '', '')
	...
	for each in ['byfname', 'regexp'] | if exists(each)
		let s:{each} = {each}
	en | endfo
	if !exists('g:fzf_index_tilde_homedir') | let g:fzf_index_tilde_homedir = 0 | en
	if !exists('g:fzf_index_newcache') | let g:fzf_index_newcache = 0 | en
	...
	if !( exists('extensions') && extensions == s:extensions )
		for each in s:extensions
			exe 'ru autoload/fzf_index/'.each.'.vim'
		endfo
	en
	" Keymaps
...
fu! s:match_window_opts()
	let s:mw_pos =
		...
		\ exists('g:fzf_index_match_window_bottom') ? ( s:mwbottom ? 'bottom' : 'top' )
		\ : 'bottom'
	let s:mw_order =
		\ s:mw =~ 'order:[^,]\+' ? matchstr(s:mw, 'order:\zs[^,]\+') :
		\ exists('g:fzf_index_match_window_reversed') ? ( s:mwreverse ? 'btt' : 'ttb' )
		\ : 'btt'
	let s:mw_max =
		\ s:mw =~ 'max:[^,]\+' ? str2nr(matchstr(s:mw, 'max:\zs\d\+')) :
		\ exists('g:fzf_index_max_height') ? s:mxheight
		\ : 10
	let s:mw_min =
...
fu! s:Open()
	...
	if s:opmul != '0' && has('signs')
		sign define fzf_indexmark text=+> texthl=fzf_indexMark
		hi def link fzf_indexMark Search
	en
	cal s:setupblank()
...
" * Clear caches {{{1
fu! fzf_index#clr(...)
	let [s:matches, g:fzf_index_new{ a:0 ? a:1 : 'cache' }] = [1, 1]
endf

fu! fzf_index#clra()
	let cadir = fzf_index#utils#cachedir()
...
" * Files {{{1
fu! fzf_index#files()
	let cafile = fzf_index#utils#cachefile()
	if g:fzf_index_newcache || !filereadable(cafile) || s:nocache(cafile)
		let [lscmd, s:initcwd, g:fzf_index_allfiles] = [s:lsCmd(), s:dyncwd, []]
		" Get the list of files
		if empty(lscmd)
			if !fzf_index#igncwd(s:dyncwd)
		...
		cal fzf_index#rmbasedir(g:fzf_index_allfiles)
		if len(g:fzf_index_allfiles) <= s:compare_lim
			cal sort(g:fzf_index_allfiles, 'fzf_index#complen')
		en
	...
	el
		...
		if !( exists('s:initcwd') && s:initcwd == s:dyncwd )
			...
			let g:fzf_index_allfiles = fzf_index#utils#readfile(cafile)
		en
	en
	cal extend(s:ficounts, { s:dyncwd : [len(g:fzf_index_allfiles), catime] })
	retu g:fzf_index_allfiles
endf
...
fu! s:GlobPath(dirs, depth)
	...
	let [dnf, depth] = [fzf_index#dirnfile(entries), a:depth + 1]
	cal extend(g:fzf_index_allfiles, dnf[1])
	if !empty(dnf[0]) && !s:maxf(len(g:fzf_index_allfiles)) && depth <= s:maxdepth
		sil! cal fzf_index#progress(len(g:fzf_index_allfiles), 1)
		cal s:GlobPath(join(map(dnf[0], 's:fnesc(v:val, "g", ",")'), ','), depth)
	en
endf

fu! fzf_index#addfile(ch, file)
	call add(g:fzf_index_allfiles, a:file)
	cal s:BuildPrompt(1)
endf
...
fu! s:UserCmd(lscmd)
	...
	if do_ign && fzf_index#igncwd(s:cwd) | retu | en
	...
	if s:usrcmdasync && v:version >= 800 && exists('*job_start')
		if exists('s:job')
			call job_stop(s:job)
		en
		let g:fzf_index_allfiles = []
		let s:job = job_start([&shell, &shellcmdflag, printf(lscmd, path)], {'callback': 'fzf_index#addfile'})
	elsei has('patch-7.4-597') && !(has('win32') || has('win64'))
		let g:fzf_index_allfiles = systemlist(printf(lscmd, path))
	el
		let g:fzf_index_allfiles = split(system(printf(lscmd, path)), "\n")
	en
	if exists('+ssl') && exists('ssl')
		let &ssl = ssl
		cal map(g:fzf_index_allfiles, 'tr(v:val, "\\", "/")')
	en
	...
	if do_ign
		if !empty(s:usrign)
			let g:fzf_index_allfiles = fzf_index#dirnfile(g:fzf_index_allfiles)[1]
		en
		if &wig != ''
			cal filter(g:fzf_index_allfiles, 'glob(v:val) != ""')
		en
	en
endf
...
fu! s:Update(str)
	...
	let lines = s:nolim == 1 && empty(str) ? copy(g:fzf_index_lines)
		\ : s:MatchedItems(g:fzf_index_lines, pat, s:mw_res)
...
fu! s:BuildPrompt(upd)
	...
	let [hiactive, hicursor, base] = s:focus
		\ ? ['fzf_indexPrtText', 'fzf_indexPrtCursor', base]
		\ : ['fzf_indexPrtBase', 'fzf_indexPrtBase', tr(base, '>', '-')]
	let hibase = 'fzf_indexPrtBase'
	" Build it
	redr
...
fu! s:SetDefTxt()
	...
	if !type(txt)
		...
		let txt = txt && !stridx(path, s:dyncwd) ? fzf_index#rmbasedir([path])[0] : ''
	en
	let s:prompt[0] = txt
endf
" ** Prt Actions {{{1
...
fu! s:PrtClearCache()
	...
	elsei s:itemtype >= len(s:coretypes)
		cal fzf_index#clr(s:statypes[s:itemtype][1])
	en
	if ct == 'mru'
		let g:fzf_index_lines = fzf_index#mrufiles#refresh()
	el
		cal fzf_index#setlines()
	en
	let s:force = 1
	cal s:BuildPrompt(1)
	unl s:force
...
fu! s:PrtDeleteMRU()
	if s:curtype() == 'mru'
		cal s:delent('fzf_index#mrufiles#remove')
	en
endf
...
fu! s:ToggleType(dir)
	let max = len(g:fzf_index_ext_vars) + len(s:coretypes) - 1
	let next = s:walker(max, s:itemtype, a:dir)
	cal fzf_index#setlines(next)
	cal fzf_index#syntax()
	cal s:PrtSwitcher()
endf
...
fu! s:SetWD(args)
	if has_key(a:args, 'args') && stridx(a:args['args'], '--dir') >= 0
		\ && exists('s:dyncwd')
		cal fzf_index#setdir(s:dyncwd) | retu
	en
	if has_key(a:args, 'dir') && a:args['dir'] != ''
		cal fzf_index#setdir(a:args['dir']) | retu
	en
	...
	for pmode in split(pmodes, '\zs')
		if fzf_index#setpathmode(pmode, spath) | retu | en
	endfo
endf
" * AcceptSelection() {{{1
fu! fzf_index#acceptfile(...)
	let useb = 0
	if a:0 == 1 && type(a:1) == 4
	...
	if exists('jmpb') && bufwinnr > 0
		...
		exe bufwinnr.'winc w'
		if j2l | cal fzf_index#j2l(j2l) | en
	...
	el
		...
		let cmd =
			...
			\ call('fzf_index#normcmd', useb ? ['b', 'bo vert sb'] : ['e'])
...
fu! s:SpecInputs(str)
	if a:str =~ '\v^(\.\.([\/]\.\.)*[\/]?[.\/]*)$' && s:spi
		let cwd = s:dyncwd
		cal fzf_index#setdir(a:str =~ '^\.\.\.*$' ?
			\ '../'.repeat('../', strlen(a:str) - 2) : a:str)
		if cwd != s:dyncwd | cal fzf_index#setlines() | en
		cal s:PrtClear()
		retu 1
	...
	elsei a:str == '?'
		...
		sil! exe 'bo vert h fzf_index-mappings' hlpwin '| norm! 0'
		retu 1
	en
	retu 0
endf
...
fu! s:AcceptSelection(action)
	...
	el
		if s:itemtype < len(s:coretypes)
			let [actfunc, type] = ['fzf_index#acceptfile', 'dict']
		el
...
fu! s:CreateNewFile(...)
	...
	en
	if base != '' | if isdirectory(fzf_index#utils#mkdir(base))
	...
	cal s:PrtExit()
	let cmd = md == 'r' ? fzf_index#normcmd('e') :
...
fu! s:MarkToOpen()
	...
	el
		...
		en
		if has('signs')
			exe 'sign place' key 'line='.line('.').' name=fzf_indexmark buffer='.s:bufnr
		en
	en
	sil! cal fzf_index#statusline()
endf
...
fu! s:OpenMulti(...)
	...
	let nopt = exists('g:fzf_index_open_multiple_files')
	if !has_marked
		let line = fzf_index#getcline()
		if line == '' | retu | en
	...
	let ncmd = ( useb ? ['b', 'bo vert sb'] : ['e', 'bo vne'] )
		\ + ( ur ? [] : ['ignruw'] )
	let fst = call('fzf_index#normcmd', ncmd)
	...
	let [ic, cmds] = [1, { 'v': ['vert sb', 'vne'], 'h': ['sb', 'new'],
		\ 't': ['tab sb', 'tabe'] }]
	let [swb, &swb] = [&swb, '']
	if md == 't' && fzf_index#tabcount() < tabpagenr()
		let s:tabct = fzf_index#tabcount()
	en
	" Open the files
	for va in mkd
		let bufnr = bufnr('^'.va.'$')
...
fu! s:compmref(...)
	...
	let [id1, id2] = [index(g:fzf_index_lines, a:1), index(g:fzf_index_lines, a:2)]
...
fu! s:mixedsort(...)
	...
	en
	let [cln, cml] = [fzf_index#complen(a:1, a:2), s:compmatlen(a:1, a:2)]
	if s:ispath
		let ms = []
		if s:res_count < 21
...
endf
" Statusline {{{2
fu! fzf_index#statusline()
	if !exists('s:statypes')
		let s:statypes = copy(s:coretypes)
		if !empty(g:fzf_index_ext_vars)
			cal map(copy(g:fzf_index_ext_vars),
	...
	el
		let item    = '%#fzf_indexMode1# '.s:ctype.' %*'
		let focus   = '%#fzf_indexMode2# '.focus.' %*'
		let byfname = '%#fzf_indexMode1# '.byfname.' %*'
		let regex   = s:regexp  ? '%#fzf_indexMode2# regex %*' : ''
		let slider  = ' <'.prv.'>={'.item.'}=<'.nxt.'>'
		let dir     = ' %=%<%#fzf_indexMode2# %{getcwd()} %*'
...
fu! fzf_index#progress(enum, ...)
	...
	el
		let &l:stl = '%#fzf_indexStats# '.a:enum.' %* '.txt.'%=%<%#fzf_indexMode2# %{getcwd()} %*'
	en
	redraws
endf
" *** Paths {{{2
...
fu! s:dircompl(be, sd)
	...
	if a:be == ''
		let dirs = fzf_index#rmbasedir(dirs)
	en
...
fu! fzf_index#igncwd(cwd)
	retu fzf_index#utils#glob(a:cwd, 0) == '' ||
		\ ( s:igntype >= 0 && s:usrign(a:cwd, getftype(a:cwd)) )
endf

fu! fzf_index#dirnfile(entries)
...
fu! s:findroot(curr, mark, depth, type)
	...
	en
	if fnd
		if !a:type | cal fzf_index#setdir(a:curr) | en
		retu [exists('markr') ? markr : a:mark, a:curr]
	elsei depth > s:maxdepth
		cal fzf_index#setdir(s:cwd)
	el
...
fu! fzf_index#setpathmode(pmode, ...)
	if a:pmode == 'c' || ( a:pmode == 'a' && stridx(s:crfpath, s:cwd) < 0 )
		if exists('+acd') | let [s:glb_acd, &acd] = [&acd, 0] | en
		cal fzf_index#setdir(s:crfpath)
		retu 1
	elsei a:pmode == 'r'
...
" Fallbacks {{{3
fu! s:glbpath(...)
	retu call('fzf_index#utils#globpath', a:000)
endf

fu! s:fnesc(...)
	retu call('fzf_index#utils#fnesc', a:000)
endf

fu! fzf_index#setlcdir()
	if exists('*haslocaldir')
		cal fzf_index#setdir(getcwd(), haslocaldir() ? 'lc!' : 'cd!')
	en
endf
" Highlighting {{{2
fu! fzf_index#syntax()
	if fzf_index#nosy() | retu | en
	for [ke, va] in items(s:hlgrps) | cal fzf_index#hicheck('fzf_index'.ke, va) | endfo
	let bgColor=synIDattr(synIDtrans(hlID('Normal')), 'bg')
	if bgColor !~ '^-1$\|^$'
		sil! exe 'hi fzf_indexLinePre guifg='.bgColor.' ctermfg='.bgColor
	en
	sy match fzf_indexNoEntries '^ == NO ENTRIES ==$'
	if hlexists('fzf_indexLinePre')
		exe "sy match fzf_indexLinePre '^".escape(get(g:, 'fzf_index_line_prefix', '>'),'^$.*~\')."'"
	en

	if s:curtype() == 'buf' && s:has_conceal
		sy region fzf_indexBufferNr     matchgroup=fzf_indexLinePre start='^>\s\+' end='\s'
		sy region fzf_indexBufferInd    concealends matchgroup=Ignore start='<bi>' end='</bi>'
		sy region fzf_indexBufferRegion concealends matchgroup=Ignore start='<bn>' end='</bn>'
			\ contains=fzf_indexBufferHid,fzf_indexBufferHidMod,fzf_indexBufferVis,fzf_indexBufferVisMod,fzf_indexBufferCur,fzf_indexBufferCurMod
		sy region fzf_indexBufferHid    concealends matchgroup=Ignore     start='\s*{' end='}' contained
		sy region fzf_indexBufferHidMod concealends matchgroup=Ignore    start='+\s*{' end='}' contained
		sy region fzf_indexBufferVis    concealends matchgroup=Ignore   start='\*\s*{' end='}' contained
		sy region fzf_indexBufferVisMod concealends matchgroup=Ignore  start='\*+\s*{' end='}' contained
		sy region fzf_indexBufferCur    concealends matchgroup=Ignore  start='\*!\s*{' end='}' contained
		sy region fzf_indexBufferCurMod concealends matchgroup=Ignore start='\*+!\s*{' end='}' contained
		sy region fzf_indexBufferPath   concealends matchgroup=Ignore start='<bp>' end='</bp>'
	en
endf
...
fu! s:highlight(pat, grp)
	...
	if !empty(a:pat) && s:ispath
		...
		cal matchadd('fzf_indexLinePre', '^>')
	en
endf

fu! s:dohighlight()
	retu s:mathi[0] && exists('*clearmatches') && !fzf_index#nosy()
endf
" Prompt history {{{2
fu! s:gethistloc()
	let utilcadir = fzf_index#utils#cachedir()
	let cache_dir = utilcadir.s:lash(utilcadir).'hist'
	retu [cache_dir, cache_dir.s:lash(cache_dir).'cache.txt']
endf

fu! s:gethistdata()
	retu fzf_index#utils#readfile(s:gethistloc()[1])
endf

fu! fzf_index#recordhist()
	let str = join(s:prompt, '')
	...
	cal fzf_index#utils#writecache(hst, s:gethistloc()[0], s:gethistloc()[1])
endf
" Signs {{{2
fu! s:unmarksigns()
...
fu! s:remarksigns()
	...
	for ic in range(1, len(s:lines))
		...
		if key > 0
			exe 'sign place' key 'line='.ic.' name=fzf_indexmark buffer='.s:bufnr
		en
	endfo
endf
...
fu! s:argmaps(md, i)
	let roh = [
		...
		\ ]
	if a:i == 2
		if !buflisted(bufnr('^'.fnamemodify(fzf_index#getcline(), ':p').'$'))
...
fu! s:delent(rfunc)
	...
	if tbrem == [] && ( has('dialog_gui') || has('dialog_con') ) &&
		...
		retu
	en
	let g:fzf_index_lines = call(a:rfunc, [tbrem])
	cal s:BuildPrompt(1)
	unl s:force
endf
...
fu! s:getenv()
	...
	let s:mrbs = fzf_index#mrufiles#bufs()
endf
...
fu! s:log(m)
	if exists('g:fzf_index_log') && g:fzf_index_log | if a:m
		let cadir = fzf_index#utils#cachedir()
		let apd = g:fzf_index_log > 1 ? '>' : ''
		sil! exe 'redi! >'.apd cadir.s:lash(cadir).'fzf_index.log'
	el
		sil! redi END
	en | en
endf
...
fu! s:openfile(cmd, fid, tail, chkmod, ...)
	let cmd = a:cmd
	if a:chkmod && cmd =~ '^[eb]$' && fzf_index#modfilecond(!( cmd == 'b' && &aw ))
		let cmd = cmd == 'b' ? 'sb' : 'sp'
	en
	let cmd = cmd =~ '^tab' ? fzf_index#tabcount().cmd : cmd
	let j2l = a:0 && a:1[0] ? a:1[1] : 0
	exe cmd.( a:0 && a:1[0] ? '' : a:tail ) s:fnesc(a:fid, 'f')
	if j2l
		cal fzf_index#j2l(j2l)
	en
	if !empty(a:tail)
		sil! norm! zvzz
	en
...
fu! s:writecache(cafile)
	if ( g:fzf_index_newcache || !filereadable(a:cafile) ) && !s:nocache()
		cal fzf_index#utils#writecache(g:fzf_index_allfiles)
		let g:fzf_index_newcache = 0
	en
endf
...
fu! s:nocache(...)
	...
	elsei s:caching > 1
		...
		elsei a:0 && filereadable(a:1)
			retu len(fzf_index#utils#readfile(a:1)) < s:caching
		en
		retu 1
	en
	retu 0
...
fu! s:insertcache(str)
	let [data, g:fzf_index_newcache, str] = [g:fzf_index_allfiles, 1, a:str]
	...
	cal s:writecache(fzf_index#utils#cachefile())
endf
" Extensions {{{2
fu! s:mtype()
	retu s:itemtype >= len(s:coretypes) ? s:getextvar('type') : 'path'
...
fu! s:execextvar(key)
	if !empty(g:fzf_index_ext_vars)
		cal map(filter(copy(g:fzf_index_ext_vars),
			\ 'has_key(v:val, a:key)'), 'eval(v:val[a:key])')
	en
endf

fu! s:getextvar(key)
	if s:itemtype >= len(s:coretypes) && len(g:fzf_index_ext_vars) > 0
		let vars = g:fzf_index_ext_vars[s:itemtype - len(s:coretypes)]
		retu has_key(vars, a:key) ? vars[a:key] : -1
	en
	retu get(g:, 'fzf_index_' . s:matchtype . '_' . a:key, -1)
endf

fu! fzf_index#getcline()
	let [linenr, offset] = [line('.'), ( s:offset > 0 ? s:offset : 0 )]
	retu !empty(s:lines) && !( offset && linenr <= offset )
		\ ? s:lines[linenr - 1 - offset] : ''
endf

fu! fzf_index#getmarkedlist()
	retu exists('s:marked') ? values(s:marked) : []
endf

fu! fzf_index#clearmarkedlist()
	let s:marked = {}
endf
...
fu! fzf_index#prtclear()
	cal s:PrtClear()
endf

fu! fzf_index#switchtype(id)
...
fu! fzf_index#hicheck(grp, defgrp)
	if !hlexists(a:grp)
		exe 'hi link' a:grp a:defgrp
	en
endf

fu! fzf_index#call(func, ...)
	retu call(a:func, a:000)
endf

fu! fzf_index#getvar(var)
	retu {a:var}
endf
"}}}1
" * Initialization {{{1
fu! fzf_index#setlines(...)
	if a:0 | let s:itemtype = a:1 | en
	cal s:modevar()
	let inits = {'fil': 'fzf_index#files()', 'buf': 'fzf_index#buffers()', 'mru': 'fzf_index#mrufiles#list()'}
	let types = map(copy(g:fzf_index_types), 'inits[v:val]')
	if !empty(g:fzf_index_ext_vars)
		cal map(copy(g:fzf_index_ext_vars), 'add(types, v:val["init"])')
	en
	let g:fzf_index_lines = eval(types[s:itemtype])
endf

" Returns [lname, sname]
fu! s:CurTypeName()
	if s:itemtype < len(s:coretypes)
		return filter(copy(s:coretypes), 'v:val[1]==g:fzf_index_types[s:itemtype]')[0]
	el
...
fu! fzf_index#init(type, ...)
  " my just getting the files
	...
	if empty(g:fzf_index_types) && empty(g:fzf_index_ext_vars)
		call fzf_index#exit()
		retu
	en
	if type(a:type) == 0
		let type = a:type
	el
		let type = index(g:fzf_index_types, a:type)
		if type == -1
			call fzf_index#exit()
			retu
		en
	en
	cal fzf_index#setlines(s:settype(type))
  " manual cleanup (taken in part from s:Close())
	unl! s:focus s:hisidx s:hstgot s:marked s:statypes s:init s:savestr
		\ s:mrbs s:did_exp
	cal s:execextvar('exit')
	cal s:log(0)
	let v:errmsg = s:ermsg
  cal fzf_index#progress('')

  return g:fzf_index_lines
